    ...
    <div>
        <form @submit.prevent="saveNotifier">
            <div class="card contain-card mb-3">
                ...
                <input v-model="notifier.enabled" type="checkbox" :id="`enable_${notifier.method}`"
                       v-bind:checked="notifier.enabled">
                ...
                <div class="card-body">
                    ...
                    <div v-if="notifier.method==='mobile'">
                        <div class="form-group row mt-3">
                            ...
                            <div class="col-sm-8">
                                <div class="input-group">
                                    <input v-bind:value="$store.getters.core.domain" type="text" class="form-control"
                                           id="statping_domain" readonly>
                                    <div class="input-group-append copy-btn">
                                        <button @click.prevent="copy($store.getters.core.domain)"
                                                class="btn btn-outline-secondary" type="button">Copy
                                        </button>
                                    </div>
                                </div>
                            </div>
                        ...
                        <div class="form-group row mt-3">
                            ...
                            <div class="col-sm-8">
                                <div class="input-group">
                                    <input v-bind:value="$store.getters.core.api_secret" type="text"
                                           class="form-control" id="apisecret" readonly>
                                    <div class="input-group-append copy-btn">
                                        <button @click.prevent="copy($store.getters.core.api_secret)"
                                                class="btn btn-outline-secondary" type="button">Copy
                                        </button>
                                    </div>
                                </div>
                    ...
                    <div v-if="notifier.method!=='mobile'" v-for="(form, index) in notifier.form" v-bind:key="index"
                         class="form-group">
                        <label class="text-capitalize">{{form.title}}</label>
                        <input v-if="formVisible(['text', 'number', 'password', 'email'], form)"
                               v-model="notifier[form.field.toLowerCase()]" :type="form.type" class="form-control"
                               :placeholder="form.placeholder">

                        <select v-if="formVisible(['list'], form)" v-model="notifier[form.field.toLowerCase()]"
                                class="form-control">
                            <option v-for="(val, k) in form.list_options" :value="val">{{val}}</option>
                        </select>

                        <span v-if="formVisible(['switch'], form)"
                              @click="notifier[form.field.toLowerCase()] = !!notifier[form.field.toLowerCase()]"
                              class="switch switch-rd-gr float-right mt-2">
                <input v-model="notifier[form.field.toLowerCase()]" type="checkbox" class="switch-sm"
                       :id="`switch_${notifier.name}_${form.field}`"
                       v-bind:checked="notifier[form.field.toLowerCase()]">
                <label class="mb-0" :for="`switch_${notifier.name}_${form.field}`"></label>
                    ...
                    <div class="row mt-4">
                        ...
                        <div class="col-sm-12">
                            ...
                            <input v-model.number="notifier.limits" type="range" name="limits" class="slider" min="1"
                                   max="300">
                            <small class="form-text text-muted">Notifier '{{notifier.title}}' will send a maximum of
                                {{notifier.limits}} notifications per hour.</small>
                        </div>
            ...
            <div v-if="notifier.data_type" class="card mb-3">
                <div class="card-header text-capitalize">
                    <font-awesome-icon @click="expanded = !expanded" :icon="expanded ? 'minus' : 'plus'"
                                       class="mr-2 pointer"/>
                ...
                <div class="card-body" :class="{'d-none': !expanded}">
                    <span class="text-muted d-block mb-3" v-if="notifier.request_info"
                          v-html="notifier.request_info"></span>
            ...
            <div class="card-body">
                <div class="row">
                    <div class="col-12 col-sm-4 mb-2 mb-sm-0 mt-2 mt-sm-0">
                        <button @click.prevent="saveNotifier" :disabled="loading" type="submit"
                    ...
                    <div class="col-12 col-md-4 mb-2 mb-sm-0 mt-2 mt-sm-0">
                        <button @click.prevent="testNotifier('success')" :disabled="loadingTest"
                    ...
                    <div class="col-12 col-md-4 mb-2 mb-sm-0 mt-2 mt-sm-0">
                        <button @click.prevent="testNotifier('failure')" :disabled="loadingTest"
                                class="btn btn-secondary btn-block text-capitalize test-notifier">
                            <font-awesome-icon v-if="loadingTest" icon="circle-notch" class="mr-2" spin/>
        ...
        <div v-if="notifier.logs" class="card mb-3">
            <div class="card-header text-capitalize">
                <font-awesome-icon @click="expanded_logs = !expanded_logs" :icon="expanded_logs ? 'minus' : 'plus'"
                                   class="mr-2 pointer"/>
                {{notifier.title}} Logs
            ...
            <div class="card-body" :class="{'d-none': !expanded_logs}">
                <div v-for="(log, i) in notifier.logs" class="alert"
        ...
        <span class="d-block small text-center mb-3">
            <span class="text-capitalize">{{notifier.title}}</span> Notifier created by <a :href="notifier.author_url"
                                                                                           target="_blank">{{notifier.author}}</a>
        </span>
